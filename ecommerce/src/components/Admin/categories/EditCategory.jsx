...
const EditCategory = () => {
  ...
  console.log(input, image, "this is inputs");
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/category/update-category-image/${id}`, {
        method: "put",
        body: formData,
      });
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const res = await fetch(`/api/category/singleCate/${id}`);
        const data = await res.json();
        if (!res.ok) {
          return toast.error(data.message);
        }
        console.log(data, "this is catedata");
        if (data.success) {
          setInput({
